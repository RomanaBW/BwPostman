...
//		echo "<div>BwPostman is now removed from your system.</div>";
	 ...
	 * @since   2.0.0
	 */
	private function _createSampleUsers() {
		...
		$query->where($_db->quoteName('title') . ' = ' . $_db->Quote('Manager'));

		$_db->setQuery($query);

		$manager_id = $_db->loadResult();
		...
		$new_group      = array('id' => 0, 'parent_id' => $manager_id, 'title' => 'BwPostmanAdmin');
		$usergroupTable->bind($new_group);
		$usergroupTable->check();
		$usergroupTable->store();
		$parent_id  = $usergroupTable->insertid();
	}
