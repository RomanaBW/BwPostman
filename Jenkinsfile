  ...
  stages {
    stage('Build') {
      steps {
        ...
        sh 'ansible-playbook ${WORKSPACE}/build/playbooks/build_package.yml --extra-vars 'workspace=${WORKSPACE} version_number=${params.VERSION_NUMBER} build=${BUILD_NUMBER} mb4_support=true'
        echo 'Unit-Tests'
