  ...
  stages {
    stage('Build') {
      steps {
        ...
        sh "ls"
        sh "sudo romana ansible-playbook ${WORKSPACE}/build/playbooks/acceptance-tester.yml -vvvv -i ${WORKSPACE}/build/playbooks/inventory/hosts --extra-vars 'project_base_dir=${WORKSPACE} version_number=${params.VERSION_NUMBER} build=${BUILD_NUMBER} test_suite=smoke'"
