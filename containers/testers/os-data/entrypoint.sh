...
Xvfb &
#Xvnc -ac -port 5900 & # old version
#java -Xmx256m -jar -Dwebdriver.chrome.driver=/usr/lib64/chromium/chromedriver /opt/selenium/selenium-server-standalone-3.0.1.jar -port 4445 &> /dev/null & #&> /data/logs/driver_log.txt
java -Xmx256m -jar -Dwebdriver.chrome.driver=/usr/lib64/chromium/chromedriver /opt/selenium/selenium-server-standalone-3.0.1.jar -port 4445 > /dev/null & #&> /data/logs/driver_log.txt
...
then
# run specific tests
codecept run acceptance 001_Backend/001_TestInstallationCest --xml report_installation.xml --html report_installation.html
