	...
	else
	{
		...
		<form action="<?php echo Route::_('index.php?option=com_bwpostman&task=register'); ?>" method="post" id="bwp_mod_form"
			...
			if ($paramsComponent->get('show_gender') == 1)
			{
				...
				?>
				<div id="bwp_mod_form_genderformat" class="form-group">
					<label id="gendermsg_mod" class="small">
			...
			{ // Show firstname-field only if set in basic parameters
				?>
				<div id="bwp_mod_form_firstnamefield" class="form-group input-group input-group-sm <?php echo $paramsComponent->get('firstname_field_obligation') ? $inputClass . '-append' : $inputClass . '-xx'; ?>">
					<?php
					...
					($paramsComponent->get('firstname_field_obligation'))
						? $required = '<div class="input-group-append add-on"><div class="input-group-text"><i class="icon-star"></i></div></div>'
						: $required = '';
					?>
					<label for="a_firstname"></label><input type="text" name="a_firstname" id="a_firstname"
						placeholder="<?php echo addslashes(Text::_('MOD_BWPOSTMANFIRSTNAME')); ?>"
						value="<?php echo $sub_firstname; ?>" class="form-control" maxlength="50" />
					<?php echo $required; ?>
				</div>
				<?php
			}
			...
			if ($paramsComponent->get('show_name_field') OR $paramsComponent->get('name_field_obligation'))
			{
				...
				?>
				<div id="bwp_mod_form_namefield" class="form-group input-group input-group-sm <?php echo ($paramsComponent->get('name_field_obligation')) ? $inputClass . '-append' : $inputClass . '-xx' ?>">
					...
					($paramsComponent->get('name_field_obligation'))
						? $required = '<div class="input-group-append add-on"><div class="input-group-text"><i class="icon-star"></i></div></div>'
						: $required = ''; ?>
					<label for="a_name"></label><input type="text" name="a_name" id="a_name" placeholder="<?php echo addslashes(Text::_('MOD_BWPOSTMANNAME')); ?>"
						value="<?php echo $sub_name; ?>" class="form-control" maxlength="50" />
					<?php echo $required; ?>
				</div>
				<?php
			...
			if ($paramsComponent->get('show_special') OR $paramsComponent->get('special_field_obligation'))
			{
				...
				?>
				<div id="bwp_mod_form_specialfield" class="form-group input-group input-group-sm <?php echo ($paramsComponent->get('special_field_obligation')) ? $inputClass . '-append' : $inputClass . '-xx' ?>">
					...
					($paramsComponent->get('special_field_obligation'))
						? $required = '<div class="input-group-append add-on"><div class="input-group-text"><i class="icon-star"></i></div></div>'
						: $required = ''; ?>
					<label for="a_special"></label><input type="text" name="a_special" id="a_special"
						...
						value="<?php echo $sub_special; ?>" class="form-control" maxlength="50" />
					<?php echo $required; ?>
				</div>
				<?php
			...
			<div id="bwp_mod_form_emailfield" class="form-group input-group input-group-sm <?php echo $inputClass; ?>-append">
				<label for="a_email"></label><input type="text" id="a_email" name="email" placeholder="<?php echo addslashes(Text::_('MOD_BWPOSTMANEMAIL')); ?>"
					value="<?php echo $sub_email; ?>" class="form-control" maxlength="100" />
				<div class="input-group-append add-on"><div class="input-group-text"><i class="icon-star"></i></div></div>
			</div>
			<?php
			if ($paramsComponent->get('show_emailformat') == 1)
			{
				...
				?>
				<div id="bwp_mod_form_emailformatfield" class="form-group">
					<label id="emailformatmsg_mod" class="small w-100">
			...
			<div class="mailinglist-title form-group hasTooltip" title="<?php echo HTMLHelper::tooltipText($mailinglists[0]->title, Text::_($mailinglists[0]->description)); ?>"><?php echo $mailinglist->title; ?>
				...
				{ ?>
					<p id="bwp_mod_form_lists" class="required mt">
						<?php echo Text::_('MOD_BWPOSTMANLISTS') . ' <sup><i class="small icon-star"></i></sup>'; ?>
					</p>
					<div id="bwp_mod_form_listsfield" class="form-group">
					<?php
					foreach ($mailinglists AS $i => $mailinglist)
					{ ?>
						<div class="a_mailinglist_item_<?php echo $i; ?> form-check">
							<label class="checkbox mailinglist-title hasTooltip" for="a_<?php echo "mailinglists$i"; ?>" title="<?php echo HTMLHelper::tooltipText($mailinglist->title, Text::_($mailinglist->description)); ?>">
				...
				if ($paramsComponent->get('disclaimer'))
				{
					...
					?>
					<div id="bwp_mod_form_disclaimer" class="form-group form-check checkbox">
					...
					echo '>' . Text::_('MOD_BWPOSTMAN_DISCLAIMER') . '</a> <sup><i class="small icon-star"></i></sup>'; ?>
				</span>
					</div>
					<?php
				} // Show disclaimer
				...
				{ ?>
					<div class="question mt">
						...
						<div class="captcha form-group input-group input-group-sm <?php echo $inputClass; ?>-append">
							<label for="a_stringQuestion"></label><input type="text" name="stringQuestion" id="a_stringQuestion"
								placeholder="<?php echo addslashes(Text::_('MOD_BWPOSTMANCAPTCHA_LABEL')); ?>" maxlength="50" class="form-control" />
							<div class="input-group-append add-on"><div class="input-group-text"><i class="icon-star"></i></div></div>
						</div>
					</div>
					<?php
				...
				if ($paramsComponent->get('use_captcha') == 2)
				{
					...
					<div class="captcha mt">
						...
						<div class="captcha form-group input-group input-group-sm <?php echo $inputClass; ?>-append">
							<label for="a_stringCaptcha"></label><input type="text" name="stringCaptcha" id="a_stringCaptcha"
				...
				<p class="mod-button-register text-right">
					<button class="button validate btn btn-outline-secondary" type="submit"><?php echo Text::_('MOD_BWPOSTMANBUTTON_REGISTER'); ?>
					</button>
				</p>
		...
		<p id="bwp_mod_form_required" class="small text-secondary">( <i class="icon-star"></i>) <?php echo Text::_('MOD_BWPOSTMANREQUIRED'); ?></p>
		<p id="bwp_mod_form_editlink" class="text-right">
			<button class="button btn btn-secondary btn-sm" onclick="location.href='<?php
