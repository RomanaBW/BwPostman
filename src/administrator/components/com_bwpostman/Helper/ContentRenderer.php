...
//			$row->text   = $row->introtext;
	...
	public function addHtmlTags(string &$text, int $id): bool
	{
		...
		PluginHelper::importPlugin('bwpostman');
        $event = new Event('onBwPostmanBeforeCustomCss', [
            'subject' => ArrayHelper::fromObject($this),
            'link'    => $newtext,
        ]);
        Factory::getApplication()->getDispatcher()->dispatch($event->getName(), $event);
	...
	public function addHTMLFooter(string &$text, int $templateId): bool
	{
		...
		if (strpos($text, '[%impressum%]') !== false)
		{
			...
			{
				PluginHelper::importPlugin('bwpostman');
                $event = new Event('onBwPostmanSubstituteLinks', [
                    'subject'         => ArrayHelper::fromObject($this),
        ...
        $event = new Event('onBwPostmanAfterObligatoryFooter', [
            'subject'    => ArrayHelper::fromObject($this),
            'text'       => $text,
	...
	public function replaceContentPlaceholders(string &$body, object $tblSendMailQueue, ?string $itemid_edit, ?string $itemid_unsubscribe, string $editlink, int $substituteLinks): void
    {
		...
		if ($editlink !== '')
		{
			...
			{
                $event = new Event('onBwPostmanSubstituteBody', [
                    'subject'         => ArrayHelper::fromObject($this),
                    'body'            => $body,
                    'itemidEdit'      => $itemid_edit,
                    'itemidUnsubscribe' => $itemid_unsubscribe,
                ]);
                Factory::getApplication()->getDispatcher()->dispatch($event->getName(), $event);
                $eventResults = $event->getArgument('result', []);
			}
